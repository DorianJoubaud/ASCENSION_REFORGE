...
Generative models, namely Generative Adversarial Networks (GANs)
...
characteristics \citep{fu2020data}. GAN-based DA methods
\citep{zhang2022data, yang2023ts, jeon2022gt, li2022tts, li2022ttsc,
tronchin2023latentaugment, seon2024least, mousavi2025transformer}
generate realistic sequences but are notoriously unstable to train
and prone to mode collapse \citep{lei2019mode}. Diffusion-based DA
methods \citep{liu2024adaptive, wang2024diffrul, solis2023,
dai2023timeddpm, yuandiffusion, naiman2024utilizing, yu2025diffat,
zhang2025time} produce stable outputs but still face challenges with
long-range dependencies, error accumulation, and slow inference
\citep{feng2024latent}. VAEs allow explicit control over the
...
Within VAE-based DA, \textbf{VaDE} \citep{jiang2016variational} and
...
% baseline)
\textbf{KoVAE} \citep{naiman2023generative} is the strongest
VAE-based generative baseline in our benchmark: it places a linear
...
samples remain confined to the density it has learned. This
difference is visible in our results, where KoVAE is the
best-behaved generative baseline, with a mean accuracy change close
to zero on ResNet and FCN, yet never positive
(\tablename~\ref{tab:main-results}), while ASCENSION's gains come
precisely from the controlled expansion of the class distributions.
...
and applies the expansion progressively over iterations. Other VAE
variants target specific priors or modalities
\citep{goubeaud2021using, dang2024vae, bogojeski2025data}. None of
these approaches explore a progressive, tunable expansion of class
representations in the latent space, as proposed in ASCENSION.
...
training set alone. The complete discussion is provided in
Appendix~\ref{app:related-extended}.
