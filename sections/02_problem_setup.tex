...
apply predefined transformations adapted from computer vision. Automated successors such as AutoAugment (AA)
\citep{cubuk2019autoaugment}, \textbf{Fast AutoAugment (FAA)}
\citep{lim2019fast}, RandAugment \citep{cubuk2020randaugment},
Deep AutoAugment \citep{zheng_deep_2022}, and Trivial
Augment \citep{muller2021trivialaugment} automate the search for these
...
characteristics \citep{fu2020data}. GAN-based DA methods for TS, such as \textbf{TTS-GAN}
\citep{li2022tts} and \textbf{LatentAugment}
\citep{tronchin2023latentaugment}, generate realistic sequences but
are notoriously unstable to train and prone to mode collapse
\citep{lei2019mode}.

Diffusion-based DA methods for TS, such as
\textbf{Time-DDPM} \citep{dai2023timeddpm}, \textbf{Diffusion-TS}
\citep{yuandiffusion} and \textbf{ImagenTime}
\citep{naiman2024utilizing}, produce stable outputs but still face
challenges with long-range dependencies, error accumulation, and slow
inference \citep{feng2024latent}.
...
space \citep{cheung2020modals}. Additionally, VAEs are
less prone to collapse than GANs and are less computationally
expensive than both alternatives \citep{thanh2020catastrophic}, which
motivates our choice of this family. In VAE-based DA, \textbf{VaDE} \citep{jiang2016variational} and
GMVAE \citep{dilokthanakul2017gmvae} structure the latent
...
ASCENSION introduces an $\alpha$-scaling mechanism that progressively
expands the per-class posterior covariances while preserving class
identity.
Outside TS classification, expanding class representations through
their covariance has precedents in image classification
\citep{wang2019implicit} and speaker recognition
\citep{wang2020data}, and covariance-based sampling has been used to
augment agronomic regression data \citep{lan2025cotton}.
Appendix~\ref{app:related-extended} extends this discussion to further
VAE variants.

% ADDED FOR TMLR (AE request: Gonen et al. as the small-sample
% benchmark; placed after the three families, which their study spans)
Across these three families, \citet{gonen2025time} benchmark four
representative models, one GAN, one VAE and two diffusion models,
under data scarcity, and report a substantial gap between the
full-data and data-scarce regimes. We examine the same regime for
ASCENSION in Section~\ref{sec:results:applicability}.
