...
automated successors such as AutoAugment (AA)
...
characteristics \citep{fu2020data}. GAN-based DA methods for TS include TimeGAN
\citep{zhang2022data}, TS-GAN \citep{yang2023ts}, GT-GAN
\citep{jeon2022gt}, \textbf{TTS-GAN} and its conditional variant
\citep{li2022tts, li2022ttsc}, \textbf{LatentAugment}
\citep{tronchin2023latentaugment}, LISGAN \citep{seon2024least} and
TTS-WGAN-GP \citep{mousavi2025transformer}. They generate realistic
sequences but are notoriously unstable to train and prone to mode
collapse \citep{lei2019mode}.

Diffusion-based DA methods for TS include ASE-DDPM
\citep{liu2024adaptive}, DiffRUL \citep{wang2024diffrul}, D3A-TS
\citep{solis2023}, \textbf{Time-DDPM} \citep{dai2023timeddpm}, \textbf{Diffusion-TS}
\citep{yuandiffusion}, \textbf{ImagenTime} \citep{naiman2024utilizing}, DiffAT
\citep{yu2025diffat} and Diff-TI \citep{zhang2025time}. They produce
stable outputs but still face challenges with long-range dependencies,
error accumulation, and slow inference \citep{feng2024latent}.
% ADDED FOR TMLR (AE request: small-sample generative benchmark)
In the first large-scale study of generative TS models under data
scarcity, \citet{gonen2025time} report a substantial gap between the
full-data and data-scarce regimes, and mitigate it by pretraining a
unified diffusion model on a large collection of datasets before
...
space, as evidenced by \cite{cheung2020modals}. Additionally, VAEs are
...
motivates our choice of this family. Within VAE-based DA, \textbf{VaDE} \citep{jiang2016variational} and
