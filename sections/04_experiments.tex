...
We use the UCR Archive of 128 univariate TSC datasets (see
Appendix~\ref{app:datasets}), retaining 102 of the 128. We exclude
datasets with missing values and datasets too small to ensure
...
sample, $n$ in total.
Baselines all add $n$ candidates to the training set, doubling it,
whereas ASCENSION keeps only the candidates that pass the
Bayes-region filter (Definition~\ref{def:bayes-region}) and therefore
adds fewer, by an amount that varies across datasets (median
acceptance rate $0.71$ to $0.73$, Appendix~\ref{app:acceptance}).
...
recommended defaults for the baselines) on 102 UCR datasets and three
classifiers. Cells give the mean (std) over three runs, Total the net
accuracy change over the 102 datasets. Arrows
$(\uparrow, \downarrow)$ mark the direction of improvement,
\textbf{bold} and \underline{underline} the best and second-best mean
per column, the unchanged count excluded since it has no preferred
direction.}
...
positive three-run mean total on any classifier: 0 of the 24
...
% related work)
KoVAE is the strongest generative baseline on ResNet, with
...
therefore not sufficient, and ASCENSION's gains come from
...
Appendix~\ref{app:cd} repeats the analysis on the best-per-dataset
configuration reference, where ASCENSION again holds the best mean
rank on all three classifiers.
...
sets aligns with \citet{gonen2025time}, who identify this
...
Moment, while on FCN every group median stays at or below $+0.24$
points, and above 50 samples per class ASCENSION is close to
neutral everywhere.
...
As shown in Section~\ref{sec:results:main}, ASCENSION is the only
method with a positive mean gain on each classifier, but a
...
and report the cumulative accuracy gain as datasets are added in
...
% (\tablename~\ref{Tab:Macro_all}); not
