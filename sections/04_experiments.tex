...
but are not among the 102. For each dataset and each
...
train one classifier for each candidate iteration count
$t \in \{0, 1, 2, 4, 8\}$ (cumulative iterations). Each iteration retrains the VAE on the union of
...
is evaluated by a fixed VAE trained on real data only, so the
Bayes region cannot drift with the synthetic data. All early
...
unaugmented baseline unchanged. Hypotheses, dataset groups and the selection
...
this protocol are not comparable to
...
small-$n$ group the deployed gain reaches $+2.72$ (ResNet) and
$+3.50$ (FCN) accuracy points, significant at the $5\%$ level, with
UMD ($+7.9$), BME ($+6.7$) and ArrowHead ($+5.3$) leading on ResNet.
The same group is flat for InceptionTime, whose small-$n$ baselines
...
classifier's baseline accuracy still leaves room for improvement on
scarce data, and a stronger inductive bias moves where that room lies,
not the mechanism.

In summary, the validation-selected rule harms no dataset group
...
classifiers for fair comparison. The discrepancy-axis analysis of
Figure~\ref{fig:cumsum-disc} is computed on the fixed-configuration
runs, and the feature-importance analysis of
Figure~\ref{fig:catch22-importance} is fitted on the
best-configuration reference runs of Appendix~\ref{app:best-config}.
...
\panelTag{FCN}\resizebox{\linewidth}{!}{\input{images/FCN_Catch22_Histo.tex}}\\
...
% TODO(Dorian): rebuttal C1 promised updating Figure 5 on the
% fixed-config runs. The original catch22+RF pipeline was not found
% on the Mac, endeavor, or the public repo. Regenerate the three
% Catch22_Histo images and re-verify the 0.64/0.54 values, then
% change the Ablation 2 opening sentence (and the app:best-config
% framing sentence) to say Figure 5 uses the fixed-config runs.
