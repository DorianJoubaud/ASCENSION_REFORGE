...
from the averaged F1--F24 features. We then examine the resulting
feature importances and, as a control, the out-of-bag $R^{2}$ of each
forest.

% REVISED FOR TMLR (rebuttal C1: the feature-importance analysis was
% recomputed on the fixed-configuration runs; its outcome is reported
% as measured)
\paragraph{Results}
No dataset trait reliably predicts where augmentation pays. Across
the 27 method--classifier forests, the out-of-bag $R^{2}$ has a
median of $0.03$ and never exceeds $0.24$, and the feature-importance
rankings are not stable across the three runs: the top-ranked feature
of a method under one run typically drops out of the top ten under
another. We therefore do not interpret individual feature
importances. This outcome is consistent with the per-dataset
variability of the gains (Appendix~\ref{app:per-dataset-std}) and
with the weak correlations of the applicability profile
(Section~\ref{sec:results:applicability}): the conditions that
favour augmentation are visible in aggregate, in the training-set
size and the class count, but no combination of the 24 measured
dataset traits predicts the per-dataset outcome.

\label{sec:analysis:discrepancy}
% REVISED FOR TMLR (transition motivated by the mechanism, with the
% direct discrepancy statistics)
Since the $\alpha$-scaling mechanism is designed to expand beyond
the training distribution, we examine the train--test discrepancy
axis directly. ASCENSION's per-dataset gain is uncorrelated with F24
(Spearman $\rho$ between $-0.04$ and $+0.24$ across the nine
classifier-run combinations, not significant in eight of the nine),
and its mean gain on the half of the datasets with the larger
discrepancy ($+1.21$, $+0.83$, $+1.01$ accuracy points on ResNet,
FCN, Moment, per-dataset gains averaged over the three runs) is
comparable to the mean gain on the other half ($+1.38$, $+0.31$,
$+0.67$). The benefit therefore does not fade as the discrepancy
grows. To visualise this, we sort the $102$ UCR datasets by
ascending discrepancy (F24, Appendix~\ref{app:discrepancy}) and
report the cumulative mean accuracy gain as datasets are added in
