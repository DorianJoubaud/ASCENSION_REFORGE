...
configuration} per method ($\alpha = 1.5$ and one iteration for
ASCENSION, authors' recommended defaults for the baselines), applied
uniformly across all 102 UCR datasets and all three classifiers, over
\textbf{three independent runs}. Cells report the mean (std) over the
three runs of the number of datasets (out of 102) with improved,
unchanged, or worsened accuracy, of the mean accuracy change
...
improvement. \textbf{Bold} and \underline{underline} mark the best and
second-best mean per column.}
...
per-dataset accuracy gain, one panel per classifier. Bars give the
mean importance over the three fixed-configuration runs, whiskers
the minimum and maximum across runs. F24 (train--test distribution
discrepancy, Appendix~\ref{app:discrepancy}) in red, F23 (train/test
size ratio) in orange.}
...
Appendix~\ref{app:datasets}). Datasets are sorted by increasing
