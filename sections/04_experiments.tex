...
expanding the initial data distribution improves DA, including
...
Following \citep{fawaz2019deep}, which identifies ResNet and
FCN (Fully Convolutional Network) as top UCR performers, we use these two
architectures, introduced for TS classification by \citep{wang2017time},
in our own implementation. We also evaluate
...
adds fewer (Appendix~\ref{app:acceptance}).
...
post-hoc analysis \citep{demsar2006statistical} at the $5\%$ level ($k = 9$ methods, $N = 102$
...
\emph{ASCENSION$_{\text{Emb}}$} uses only this auxiliary classifier
...
output. Both variants test augmentation directly in the
representation space the contrastive loss shapes, which is why they
carry the component ablation. \tablename~\ref{tab:embedded} reports
both alternative configurations. The ablation studies in
...
(see metric definitions in Appendix~\ref{app:discrepancy:form}). A full
...
in any one quartile, supports our central hypothesis:
