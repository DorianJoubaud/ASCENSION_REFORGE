...
iteration rule, over 78 UCR datasets and 3 seeds (234 dataset--seed
pairs per classifier), with $t$ statistics over pairs in parentheses.
The small-$n$ group (training sets of at most 60 samples, 11 datasets)
was pre-declared before execution. Abstention is the share of pairs
where the rule deploys the unaugmented baseline ($t^{*}=0$).}
\label{tab:valselect}
\begin{tabular}{@{}lccc@{}}
\toprule
\bf Classifier & \bf All pairs & \bf Small-$n$ group & \bf Abstention \\
\midrule
...
\paragraph{Results}
\tablename~\ref{tab:valselect} reports the deployed gains for
...
\begin{table}[H]
\centering
\caption{Deployed gain per dataset group (accuracy points, balanced
accuracy for the imbalanced group), with $t$ statistics and counts
over dataset--seed pairs. 3 seeds per dataset.}
...
classifiers. The pairs with $t^{*} > 0$ use the whole range (on
ResNet, 41/29/32/18 pairs at $t = 1/2/4/8$), so the selected iteration
count genuinely varies per dataset rather than being a constant
choice. These observations about baseline accuracy levels are post-hoc
...
\begin{figure}[H]
\centering
\resizebox{0.75\textwidth}{!}{\input{images/boxPlot_risk.tex}}
...
\caption{Accuracy gain ($\overline{Acc}_{\Delta_1 Step}$) after first augmentation step per dataset for the $\alpha$-scaling mechanism and clustering loss ablation studies, along with best pair (iter., $\alpha$) identified per dataset, and the train--test discrepancy ratio (F24, column Ratio) with the intra-class dispersions of the test and train sets}\label{Tab:app:Ablation_clustering_alpha} 
...
\caption{Accuracy gain ($\overline{Acc}_{\Delta_1 Step}$) after first augmentation step per dataset for the $\alpha$-scaling mechanism and clustering loss ablation studies, along with best pair (iter., $\alpha$) identified per dataset, and the train--test discrepancy ratio (F24, column Ratio) with the intra-class dispersions of the test and train sets}
...
Figures~\ref{fig:feature-importance-all-resnet},
\ref{fig:feature-importance-all-fcn} and
\ref{fig:feature-importance-all-moment} extend
Figure~\ref{fig:catch22-importance} to VaDE, FAA, TTS-GAN, KoVAE,
Time-DDPM, LA, ImagenTime and Diffusion-TS, for ResNet, FCN and
Moment respectively. The protocol is the one of
...
\begin{figure}[p]
\centering
\includegraphics[width=\linewidth]{figures/feature_importance_all_resnet.pdf}
\caption{Feature importance of F1--F24 for predicting the per-dataset
accuracy gain of each DA method with ResNet. Bars give the mean
importance over the three fixed-configuration runs, whiskers the
minimum and maximum across runs. F24 (train--test distribution
discrepancy) in red, F23 (train/test size ratio) in orange.}
\label{fig:feature-importance-all-resnet}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\linewidth]{figures/feature_importance_all_fcn.pdf}
\caption{Feature importance of F1--F24 for predicting the per-dataset
accuracy gain of each DA method with FCN. Bars give the mean
importance over the three fixed-configuration runs, whiskers the
minimum and maximum across runs. F24 (train--test distribution
discrepancy) in red, F23 (train/test size ratio) in orange.}
\label{fig:feature-importance-all-fcn}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\linewidth]{figures/feature_importance_all_moment.pdf}
\caption{Feature importance of F1--F24 for predicting the per-dataset
accuracy gain of each DA method with Moment. Bars give the mean
importance over the three fixed-configuration runs, whiskers the
minimum and maximum across runs. F24 (train--test distribution
discrepancy) in red, F23 (train/test size ratio) in orange.}
\label{fig:feature-importance-all-moment}
\end{figure}
